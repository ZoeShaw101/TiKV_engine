package com.alibabacloud.polar_race.engine.common.core.stats;
...
public class FileStatsCollector extends Thread {
    ...
    public FileStatsCollector(DBStats stats, List<LevelQueue>[] levelQueueLists) {
        this.stats = stats;
    ...
    public void run() {
        while (!stop) {
            try {
                for (int level = 0; level <= LSMDB.MAX_LEVEL; ++level) {
                    long fileSize = 0;
                    int fileCount = 0;
